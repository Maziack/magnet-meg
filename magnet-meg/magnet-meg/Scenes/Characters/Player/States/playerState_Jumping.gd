...
@export var wall_jump_cooldown: Timer
...
func enter(_previous_state_path: String = "", _data: Dictionary = {}) -> void:
	...
	if _previous_state_path == WALLPRESSING:
		...
		player.velocity.x = player.wall_jump_pushback * player.get_wall_normal().x ### USE PLAYER.WALLDETECT_L/R
	...
	wall_jump_cooldown.wait_time = player.input_buffer
...
func physics_update(_delta: float) -> void:
	...
	if player.velocity.y >= 0:
		...
		input_buffer.start()
	if wall_jump_cooldown.is_stopped():
		if ((player.wall_jump_count > 0) or (direction_x != 0)) and (player.wall_detect_left.is_colliding() or player.wall_detect_right.is_colliding()): 
